...
export class MessageHandler {
  ...
  async handle(client: Client, message: Message): Promise<void> {
    ...
    try {
      ...
      if (body === '1') {
        await message.reply(this.menuService.getOrientacoes());
        return;
      }
      ...
      if (body === '3') {
        await message.reply(this.menuService.getContato());
        return;
      }

      if (body === '4') {
        await message.reply(this.menuService.getHorario());
        return;
      }

      if (body === '5') {
        await message.reply(this.menuService.getDireitos());
        return;
      }
